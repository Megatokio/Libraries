...
static void test1(uint& num_tests, uint& num_errors)
{
	...
	END
		...
		TRY for (int i = 9; i < 20; i++) map.add(i, i * 10); // note: this grows the hash map
	assert(map.count() == 20);
	...
	END
		...
		TRY map.add(33, 330); // hash collision
	assert(map.count() == 3);
	assert(map[0] == 0);
	assert(map[32] == 320);
	assert(map[33] == 330);
	END
		...
		TRY for (int i = 0; i <= 80; i += 16) map.add(i, i * 10);
	assert(map.count() == 7);
	for (int i = 0; i <= 80; i += 16) assert(map.get(i, -1) == i * 10);
	assert(map[33] == 330);
	END

		TRY map.remove(33); // in the middle of a thread: 0-32-33-64
	assert(map.count() == 6);
	for (int i = 0; i <= 80; i += 16) assert(map.get(i, -1) == i * 10);
	assert(!map.contains(33));
	END

		TRY map.remove(80); // at end of a thread: 16-48-80
	assert(map.count() == 5);
	for (int i = 0; i < 80; i += 16) assert(map.get(i, -1) == i * 10);
	assert(!map.contains(80));
	END

		TRY				HashMap<cstr, uint>
						a;
	HashMap<cstr, uint> b;
...
static void test3(uint& num_tests, uint& num_errors)
{
	...
	for (uint i = a.count(); i--;) { assert(eq(map[a[i]], tostr(i))); }
