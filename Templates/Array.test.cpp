...
static void test_prerequisites(uint& num_tests, uint& num_errors)
{
	...
	TRY int array1[] = {6, 4, 9, 2, 1, 0, 4, 8, 7, 6, 2, 1, 9, 9, 1};
	...
	assert(memcmp(array1, sorted, size) == 0);
	rsort(&array2[0], &array2[count]);
	assert(memcmp(array2, rsorted, size) == 0);
	sort(&array3[0], &array3[count], gt);
	assert(memcmp(array3, sorted, size) == 0);
	sort(&array4[0], &array4[count], lt);
	assert(memcmp(array4, rsorted, size) == 0);
	END

		TRY const char q[]		 = {'H', 'e', 'l', 'l', 'o', ' ', 'W', 'o', 'r', 'l', 'd', '!',
									10,	 'D', 'i', 'e', ' ', 'K', 'u', 'h', '.', 10,  0};
	...
	END
		...
		TRY cstr q[]  = {"Anton", "\tfoo", nullptr, "\t", "anton", "Antonov"};
	...
	memcpy(a, q, size);
	sort(a, a + count);
	assert(memcmp(a, sorted, size) == 0);
	a = new cstr[count];
	memcpy(a, q, size);
	rsort(a, a + count);
	assert(memcmp(a, rsorted, size) == 0);
	a = new cstr[count];
	memcpy(a, q, size);
	sort(a, a + count, gt);
	assert(memcmp(a, sorted, size) == 0);
...
static void test1(uint& num_tests, uint& num_errors)
{
	...
	TRY		assert(array.count() == 0 && array2 == Array<int>() << 6 << 5 << 9 << 8);
	END
	...
	END
		...
		TRY array.removerange(2, 4);
	assert(array == Array<int>() << 7 << 6 << 3 << 2);
	END

		TRY array.insertrange(2, 4);
	assert(array == Array<int>() << 7 << 6 << 0 << 0 << 3 << 2);
	END

		TRY array.removerange(4, 99);
	...
	END
		...
		TRY array.append(Array<int>(array));
	...
	END
		...
		TRY int m = Array<int>::MAGIC * 0x10001;
	Array<int>	a;
...
static void test2(uint& num_tests, uint& num_errors)
{
	...
	END
		...
		TRY array.appendifnew("44");
	assert(array.count() == 4);
	END

		TRY array.appendifnew("22");
	assert(array.count() == 4);
	END

		TRY array.removeitem("33");
