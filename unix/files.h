...
inline 	bool	exists_node		(cstr path, bool resolve_last_symlink=1) noexcept	{ return classify_file(path,resolve_last_symlink)!=s_none; }
inline 	bool	is_file			(cstr path, bool resolve_last_symlink=1) noexcept	{ return classify_file(path,resolve_last_symlink)==s_file; }
inline 	bool	is_dir			(cstr path, bool resolve_last_symlink=1) noexcept	{ return classify_file(path,resolve_last_symlink)==s_dir; }
...
inline 	void 	read_file		(cstr path, Array<cstr>& a, uint32 maxsize=1<<28) 			{ read_file(path,reinterpret_cast<Array<str>&>(a),maxsize); }
...
#endif
inline 	int	set_file_readable	(cstr path, mode_t who = OWNER|GROUP) { return set_file_permissions(path,who,READABLE); }
inline 	int	set_file_writable	(cstr path, mode_t who = OWNER|GROUP) { return set_file_permissions(path,who,WRITABLE); }
inline 	int	set_file_executable	(cstr path, mode_t who = OWNER|GROUP) { return set_file_permissions(path,who,EXECUTABLE); }
