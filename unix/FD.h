...
TPL	T		read			()						{ T n; read_bytes(_ptr(&n),sizeof(T)); return n; }
	...
	uint32	write_bytes		(void const* p, uint32 sz);
TPL uint32	write_data		(T const* p, uint32 cnt) { return write_bytes(p,cnt*sizeof(T)); }	// returns bytes written
TPL uint32	write_data		(T const* p);		 //	{ return write_bytes(p,sizeof(T)); }		// returns bytes written

TPL	uint32	write			(T const& n)			{ return write_bytes(_cptr(&n),sizeof(T)); } // ref wg. auto propagate
...
// always revert byte order:
	...
	uint32	write_bytes_reverted(void const* p, uint n);	// write bytes in reverted order

	uint32	read_data_reverted(void* p, uint n, uint sz);	// read bytes and revert order
	uint32	write_data_reverted(void const* p, uint n, uint sz);	// write bytes in reverted order
...
// read/write network byte order, msb first, big endian:
	...
	uint32	write_bytes_x	(void const* p, uint sz){ return(write_bytes_reverted(p,sz)); }
TPL	uint32	read_data_x		(T* p, uint cnt)		{ return(read_data_reverted(p,cnt,sizeof(T))); }
TPL	uint32	write_data_x	(T const* p, uint cnt)	{ return(write_data_reverted(p,cnt,sizeof(T))); }
#else
	uint32	read_bytes_x	(void* p, uint sz)		{ return(read_bytes(p,sz)); }
	uint32	write_bytes_x	(void const* p, uint sz){ return(write_bytes(p,sz)); }
TPL	uint32	read_data_x		(T* p, uint cnt)		{ return(read_data(p,cnt)); }
TPL	uint32	write_data_x	(T const* p, uint cnt)	{ return(write_data(p,cnt)); }
#endif

TPL uint32	read_x			(T& n)					{ return read_bytes_x(&n,sizeof(T)); }
TPL	uint32	write_x			(T const& n)			{ return write_bytes_x(&n,sizeof(T)); }
	...
	float32	read_float32_x	()						{ float32 f; read_bytes_x(&f,4); return f; }
	float64	read_float64_x	()						{ float64 f; read_bytes_x(&f,8); return f; }
	float128 read_float128_x()						{ float128 f;read_bytes_x(&f,16);return f; }
...
// read/write intel byte order (lsb first, little endian)
	...
	uint32	write_bytes_z	(void const* p, uint sz){ return(write_bytes(p,sz)); }
TPL	uint32	read_data_z		(T* p, uint cnt)		{ return(read_data(p,cnt)); }
TPL	uint32	write_data_z	(T const* p, uint cnt)	{ return(write_data(p,cnt)); }
#else
	uint32	read_bytes_z	(void* p, uint sz)		{ return(read_bytes_reverted(p,sz)); }
	uint32	write_bytes_z	(void const* p, uint sz){ return(write_bytes_reverted(p,sz)); }
TPL	uint32	read_data_z		(T* p, uint cnt)		{ return(read_data_reverted(p,cnt,sizeof(T))); }
TPL	uint32	write_data_z	(T const* p, uint cnt)	{ return(write_data_reverted(p,cnt,sizeof(T))); }
#endif

TPL uint32	read_z			(T& n)					{ return read_bytes_z(_ptr(&n),sizeof(T)); }
TPL	uint32	write_z			(T const& n)			{ return write_bytes_z(_cptr(&n),sizeof(T)); }
	...
	float32	read_float32_z	()						{ float32 f; read_bytes_z(&f,4); return f; }
	float64	read_float64_z	()						{ float64 f; read_bytes_z(&f,8); return f; }
	float128 read_float128_z()						{ float128 f;read_bytes_z(&f,16);return f; }
