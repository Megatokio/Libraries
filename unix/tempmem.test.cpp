...
static uint64 const multiplier = 6364136223846793005u;
static uint64 const increment  = 1442695040888963407u;		// Or an arbitrary odd constant

static inline uint32 rotr32 (uint32 x, uint r)
{
	return x >> r | x << (-r & 31);
}
...
void test_tempmem (uint& num_tests, uint& num_errors)
{
	...
	for (uint i=0; i<NELEM(threads); i++)
	{
		pthread_create(&threads[i],nullptr,do_test_tempmem,nullptr);
	}
	do_test_tempmem(nullptr);
	for (uint i=NELEM(threads); i--; )
	{
