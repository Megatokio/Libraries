...
//
__printflike(1, 0) __attribute__((__noreturn__)) void panic(cstr fmt, va_list);

__printflike(1, 2) __attribute__((__noreturn__)) void panic(cstr fmt, ...)
{
	va_list va;
	va_start(va, fmt);
	panic(fmt, va);
...
__attribute__((__noreturn__)) void panic(int err) { panic("%s", strerror(err)); }

__attribute__((__noreturn__)) static void panic(cstr where, int err) { panic("%s: %s", where, strerror(err)); }
...
static void lock() noexcept
{
	...
	if (e) panic("LogFile:lock failed");
}
static void unlock() noexcept
{
	int e = pthread_mutex_unlock(&mutex);
	if (e) panic("LogFile:unlock failed");
}
#else
...
static LogFile* getLogfile(double now)
{
	if (now >= logrotate_when) // => init or rotate log
	{
		if (logrotate_when == 0.0) init();					// first call
			...
			openLogfile(
				logdir, logrotate, max_logfiles,
				log2console, // rotate log
...
static void write2log(uint thread_id, double when, uint indent, cstr msg)
{
	...
	if (timestamp_with_date)
	{
		...
		uint y = uint(dt.tm_year + 1900);
		uint m = uint(dt.tm_mon + 1);
		uint d = uint(dt.tm_mday);
	...
	else
	{
		...
		else sprintf(timestamp, fmt00, hour % 24, minu % 60, uint(sec % 60));
	}
...
__attribute__((__noreturn__)) void panic(cstr fmt, va_list va)
{
...
//
static void delete_logfile(void* logfile) { delete reinterpret_cast<LogFile*>(logfile); }
...
static void init_once(void)
{
	...
	{
		openLogfile(
			nullptr /*dirpath*/, LOGFILE_ROTATION, LOGFILE_MAX_LOGFILES, LOGFILE_LOG_TO_CONSOLE,
...
LogFile::LogFile() noexcept : indent(0), when(0.0), composition(0), repetitions(0)
{
	msg[0] = 0;
...
void openLogfile(
	cstr dirpath, LogRotation logrotate, uint max_logfiles, bool log2console, bool with_date, bool with_msec)
{
	...
	{
		delete[] logdir;
		logdir = new char[strlen(dirpath) + 1];
		strcpy(ptr(logdir), dirpath);
	}
	...
	case DAILY:
		snprintf(
			filepath, NELEM(filepath), "%s%s-%04u-%02u-%02u.log", dirpath, APPL_NAME, dt.tm_year + 1900, dt.tm_mon + 1,
			dt.tm_mday);
	...
	case MONTHLY:
		...
		snprintf(filepath, NELEM(filepath), "%s%s-%04u-%02u.log", dirpath, APPL_NAME, dt.tm_year + 1900, dt.tm_mon + 1);
		dt.tm_mon += 1;
		dt.tm_mday	   = 1;
	...
	case WEEKLY:
	{
		...
		snprintf(
			filepath, NELEM(filepath), "%s%s-%04u-week-%02u.log", dirpath, APPL_NAME, dt.tm_year + 1900,
			1 + (dt.tm_yday + 3) / 7);
...
void log(cstr format, va_list va) { getLogfile(0.0)->vaadd(format, va); }
