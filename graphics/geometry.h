...
struct Dist
{
	...
	Dist& operator+= (const Dist& d) noexcept { dx += d.dx; dy += d.dy; return *this; }
	Dist& operator-= (const Dist& d) noexcept { dx -= d.dx; dy -= d.dy; return *this; }
	Dist& operator/= (coord n) noexcept		  { dx /= n; dy /= n; return *this; }
	Dist& operator*= (coord n) noexcept		  { dx *= n; dy *= n; return *this; }
...
struct Point
{
	...
	Point& operator+= (const Dist<coord>& d) noexcept { x += d.dx; y += d.dy; return *this; }
	Point& operator-= (const Dist<coord>& d) noexcept { x -= d.dx; y -= d.dy; return *this; }
...
struct Rect
{
	...
	{
		if (sz.width<0)  { p1.x += sz.width;  sz.width  = -sz.width;  }
		if (sz.height<0) { p1.y += sz.height; sz.height = -sz.height; }
	}
	...
	void setLeft(T x) noexcept   { sz.width  += p1.x-x; p1.x = x; }
	void setTop(T y) noexcept    { sz.height += p1.y-y; p1.y = y; }
	...
	void setTopLeft(const Point<T>& p) noexcept     { setTop(p.y);    setLeft(p.x);  }
	void setTopRight(const Point<T>& p) noexcept    { setTop(p.y);    setRight(p.x); }
	void setBottomLeft(const Point<T>& p) noexcept  { setBottom(p.y); setLeft(p.x);  }
	void setBottomRight(const Point<T>& p) noexcept { setBottom(p.y); setRight(p.x); }

	// grow / shrink box at all 4 corners:
	void grow(T d) noexcept { p1.x-=d; p1.y-=d; sz.width+=2*d; sz.height+=2*d; }
	...
	bool encloses(const Rect& q) const noexcept
	{
		...
		return q.left() >= left() && q.right() <= right() &&
			   q.top() >= top() && q.bottom() <= bottom();
	}
	bool intersects(const Rect& q) const noexcept
	{
		...
		return top() < q.bottom() && bottom() > q.top() &&
			   left() < q.right() && right() > q.left();
	}
