...
class GifEncoder
{
	...
	int depth, clear_code, eof_code, running_code, running_bits, max_code_plus_one, current_code, shift_state,
...
*/
	void writeScreenDescriptor(uint16 w, uint16 h, const Colormap& cmap, uint8 aspect = 0) THF
	{
...
*/
	...
	void writePixelmap(int bits_per_pixel, const Pixelmap& pixel) THF;
...
*/
	...
	{
		writeImageDescriptor(pm.x1(), pm.y1(), pm.width(), pm.height(), flags);
	}
	void writeImageDescriptor(const Pixelmap* pm, uint8 flags = 0) { writeImageDescriptor(*pm, flags); }
...
*/
	void writeColormap(const Colormap& cmap) THF { fd.write_bytes(cmap.getCmap(), cmap.cmapByteSize()); }
	void writeColormap(const Colormap* cmap) THF { writeColormap(*cmap); }
...
*/
	void writeImage(Pixelmap* pm, const Colormap* cm) THF;
	void writeImage(Pixelmap& pm, const Colormap& cm) THF { writeImage(&pm, &cm); }
