...
void test_cstrings(uint& num_tests, uint& num_errors)
{
	...
	TRY assert(eq(emptystr, ""));
	END

		TRY assert(is_space(' '));
	char	c = 0;
	do {
		assert(is_space(c) == (c <= ' ' && c > 0));
	}
	while (++c);
	END

		TRY assert(is_uppercase('A'));
	char	c = 0;
	do {
		assert(is_uppercase(c) == (c >= 'A' && c <= 'Z'));
	}
	while (++c);
	END

		TRY assert(is_lowercase('a'));
	char	c = 0;
	do {
		assert(is_lowercase(c) == (c >= 'a' && c <= 'z'));
	}
	while (++c);
	END

		TRY assert(is_letter('a'));
	char	c = 0;
	do {
		assert(is_letter(c) == (to_upper(c) >= 'A' && to_upper(c) <= 'Z'));
	}
	while (++c);
	END

		TRY assert(to_lower('A') == 'a');
	char	c = 0;
	do {
		assert((c >= 'A' && c <= 'Z') ? to_lower(c) == c + 32 : to_lower(c) == c);
	}
	while (++c);
	END

		TRY assert(to_upper('a') == 'A');
	char	c = 0;
	do {
		assert((c >= 'a' && c <= 'z') ? to_upper(c) == c - 32 : to_upper(c) == c);
	}
	while (++c);
	END

		TRY assert(is_bin_digit('0'));
	char	c = 0;
	do {
		assert(is_bin_digit(c) == (c == '0' || c == '1'));
	}
	while (++c);
	END

		TRY assert(is_oct_digit('0'));
	char	c = 0;
	do {
		assert(is_oct_digit(c) == (c >= '0' && c <= '7'));
	}
	while (++c);
	END

		TRY assert(is_dec_digit('0'));
	char	c = 0;
	do {
		assert(is_dec_digit(c) == (c >= '0' && c <= '9'));
	}
	while (++c);
	END

		TRY assert(is_hex_digit('0'));
	char	c = 0;
	do {
		assert(is_hex_digit(c) == ((c >= '0' && c <= '9') || (c >= 'A' && c <= 'F') || (c >= 'a' && c <= 'f')));
	}
	while (++c);
	END

		TRY for (char c = 77; ++c != 77;)
	{
	...
	END
		...
		TRY assert(dec_digit_value(('0') == 0));
	END

		TRY assert(hex_digit_value(('0') == 0));
	...
	char c = 0;
	do {
		assert((hex_digit_value(c) >= 36) == (no_dec_digit(c) && !is_letter(c)));
	}
	while (++c);
	END

		TRY assert(hexchar(0) == '0');
	assert(hexchar(9) == '9');
	assert(hexchar(10) == 'A');
	assert(hexchar(15) == 'F');
	assert(hexchar(16) == '0');
	END
		...
		TRY assert(strLen("123") == 3);
	END

		TRY assert(lt("123", "23"));
	END

		TRY assert(gt("234", "1234"));
	END

		TRY assert(gt_tolower("big", "Apple"));
	END

		TRY assert(eq("123", "123"));
	END

		TRY assert(ne("123", "124"));
	END

		TRY char s[] = "asdfghJKlJKL";
	...
	END
		...
		TRY assert(startswith("Asdfg", "Asd"));
	END

		TRY assert(endswith("Asdfg", "fg"));
	END

		TRY assert(contains("Asdfg", "sd"));
	END

		TRY assert(isupperstr("ANTON_01;"));
	END

		TRY assert(islowerstr("anton_01;"));
	END
	...
	assert(s1[20] == 0);
	str s2 = tempstr(1);
	assert(s2 == s1 - 2); // not required but expected
	END
	...
	END
		...
		TRY assert(eq(spacestr(5, 'x'), "xxxxx"));
	END

		TRY assert(eq(spaces(5), "     "));
	assert(spaces(5) == spaces(10) + 5); // not required but expected: short strings are in static const
	END

		TRY assert(eq(whitestr("\tFoo Bar\n"), "\t       \n"));
	END

		TRY static const char s1[4] = "123";
	...
	END
		...
		TRY static const cstr s = "12345678";
	assert(eq(substr(s + 2, s + 5), "345"));
	END

		TRY assert(eq(mulstr("123", 4), "123123123123"));
	END

		TRY assert(eq(catstr("123", "45"), "12345"));
	...
	END
		...
		TRY assert(eq(midstr("Antonov", 2, 3), "ton"));
	assert(eq(midstr("Antonov", 2), "tonov"));
	END

		TRY assert(eq(leftstr("Antonov", 3), "Ant"));
	END

		TRY assert(eq(rightstr("Antonov", 3), "nov"));
	END

		TRY assert(lastchar("123") == '3');
	END

		TRY str s = dupstr("AntoNov 123");
	toupper(s);
	...
	END
		...
		TRY assert(eq(upperstr("AnToNoV 123;"), "ANTONOV 123;"));
	END

		TRY assert(eq(lowerstr("AnToNoV 123;"), "antonov 123;"));
	END

		TRY assert(eq(replacedstr("Beet", 'e', 'o'), "Boot"));
	END

		TRY assert(eq(escapedstr("123\tabc\n"), "123\\tabc\\n"));
	END

		TRY assert(eq(unescapedstr("123\\tabc\\n"), "123\tabc\n"));
	END

		TRY assert(eq(quotedstr("123\tabc\n"), "\"123\\tabc\\n\""));
	END

		TRY assert(eq(unquotedstr("\"123\\tabc\\n\""), "123\tabc\n"));
	END

		TRY assert(eq(tohtmlstr("a<int&>\n"), "a&lt;int&amp;&gt;<br>"));
	END

		TRY assert(eq(fromhtmlstr("a&lt;int&amp;&gt;<br>"), "a<int&>\n"));
	assert(eq(fromhtmlstr("foo<br>a&lt;int&amp;&gt;<br>"), "foo\na<int&>\n"));
	END

		TRY static const uchar s[] = {'a', 0xC4, 0xFF, 0};
	...
	END
		...
		TRY static const uchar s[] = {'a', 0xC4, 0xFF, ' ', '\n', 0};
	...
	END
		...
		TRY assert(eq(base64str("Die Fo01[]"), "RGllIEZvMDFbXQ=="));
	assert(eq(base64str("oDie Fo01[]"), "b0RpZSBGbzAxW10="));
	assert(eq(base64str("o Die Fo01[]"), "byBEaWUgRm8wMVtd"));
	END

		TRY assert(eq(unbase64str("byBEaWUgRm8wMVtd"), "o Die Fo01[]"));
	assert(eq(unbase64str("b0RpZSBGbzAxW10="), "oDie Fo01[]"));
	assert(eq(unbase64str("RGllIEZvMDFbXQ=="), "Die Fo01[]"));
	END

		TRY assert(eq(croppedstr("\t hallo\n"), "hallo"));
	END

		TRY assert(eq(detabstr(" \thallo\t\n", 4), "    hallo   \n"));
	END

		TRY assert(eq(usingstr("foo%s", "bar"), "foobar"));
	END

		TRY assert(eq(tostr(1.5f), "1.5"));
	...
	END
		...
		TRY assert(eq(binstr(0x2345), "01000101"));
	...
	END
		...
		TRY assert(eq(hexstr(0x2345, 8), "00002345"));
	...
	END
		...
		TRY char s1[] = "Die Kuh lief um den Teich herum zum Wasser.\n";
	Array<str>	 array;
	split(array, s1, ' ');
	...
	END
		...
		TRY assert(ucs1::is_uppercase('A'));
	...
	END
		...
		TRY assert(ucs1::is_lowercase('a'));
	...
	END
		...
		TRY static const char s1[] = "\t1hGfr&%<'";
	...
	END
		...
		TRY assert(eq(utf8::fromhtmlstr("a&lt;int&amp;&gt;<br>"), "a<int&>\n"));
	assert(eq(utf8::fromhtmlstr("foo<br>a&lt;int&amp;&gt;<br>"), "foo\na<int&>\n"));
	END

		TRY assert(eq(utf8::detabstr(" \thällö\t\n", 4), "    hällö   \n"));
	END
}
