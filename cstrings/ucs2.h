...
inline bool gc_in_range(GeneralCategory a, ucs2char c, GeneralCategory e)
{
	return uint16(general_category(c) - a) <= uint16(e - a);
}
inline bool gc_in_range(GeneralCategory a, GeneralCategory p, GeneralCategory e)
{
	return uint16(p - a) <= uint16(e - a);
}
...
inline bool is_control(ucs2char c) noexcept
{
...
inline bool is_dec_digit(ucs2char c) noexcept
{
...
inline bool has_numeric_value(ucs2char c) noexcept
{
	return c < 0xb2 ? is_in_range('0', c, '9') : gc_in_range(GcNumber, c, GcOtherNumber);
}

// decimal digits, roman numbers
inline bool is_number_letter(ucs2char c) noexcept
{
...
inline ucs2char hexchar(int n) noexcept
{
	n &= 15;
	return ucs2char((n >= 10 ? 'A' - 10 : '0') + n);
}
