...
void Names::serialize(SerialDevice* fd, void*) const
{
	uint32 a = num_stdnames;
	uint32 e = hashmap.count();

	fd->write<uint16>(MAGIC);
	fd->write(a);
	fd->write(e);
...
void Names::deserialize(SerialDevice* fd, void*)
{
	uint32 m = fd->read<uint16>();
	uint32 a = fd->read<uint32>();
	uint32 e = fd->read<uint32>();

	if (m != MAGIC) throw data_error;
	if (a != num_stdnames) throw data_error;
