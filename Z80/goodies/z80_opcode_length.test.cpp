...
static const uint8 z80_ix_len[256] = {
	2, 4, 2, 2, 2, 2, 3,
	2, //	NOP,		LD_BC_NN,	LD_xBC_A,	INC_BC,		INC_B,		DEC_B,		LD_B_N,		RLCA,
	2, 2, 2, 2, 2, 2, 3,
	2,						//	EX_AF_AF,	ADD_HL_BC,	LD_A_xBC,	DEC_BC,		INC_C,		DEC_C,		LD_C_N,		RRCA,
	...
	2, 2, 4, 4, 4, 2, 3,
	2, //	RET_NZ,		POP_BC,		JP_NZ,		JP,			CALL_NZ,	PUSH_BC,	ADD_N,		RST00,
	2, 2, 4, 0, 4, 4, 3,
	2, //	RET_Z,		RET,		JP_Z,		PFX_CB,		CALL_Z,		CALL,		ADC_N,		RST08,
	2, 2, 4, 3, 4, 2, 3,
	2, //	RET_NC,		POP_DE,		JP_NC,		OUTA,		CALL_NC,	PUSH_DE,	SUB_N,		RST10,
	2, 2, 4, 3, 4, 1, 3,
	2, //	RET_C,		EXX,		JP_C,		INA,		CALL_C,		PFX_IX,		SBC_N,		RST18,
	2, 2, 4, 2, 4, 2, 3,
	2, //	RET_PO,		POP_HL,		JP_PO,		EX_HL_xSP,	CALL_PO,	PUSH_HL,	AND_N,		RST20,
	2, 2, 4, 2, 4, 1, 3,
	2, //	RET_PE,		JP_HL,		JP_PE,		EX_DE_HL,	CALL_PE,	PFX_ED,		XOR_N,		RST28,
	2, 2, 4, 2, 4, 2, 3,
	2, //	RET_P,		POP_AF,		JP_P,		DI,			CALL_P,		PUSH_AF,	OR_N,		RST30,
	2, 2, 4, 2, 4, 1, 3,
	2, //	RET_M,		LD_SP_HL,	JP_M,		EI,			CALL_M,		PFX_IY,		CP_N,		RST38
};
...
TEST_CASE("z80_opcode_length")
{
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(Cpu8080, z) == i8080_len[i]);
		}
	}
	...
	SUBCASE("")
	{
		...
		{
			uint8 z[4] = {uint8(i), 0, 0, 0};
			if (z80_len[i]) CHECK(opcode_length(CpuZ80, z) == z80_len[i]);
		}
	}
	...
	SUBCASE("")
	{
		...
		{
			uint8 z[4] = {uint8(i), 0, 0, 0};
			if (z80_len[i]) CHECK(opcode_length(CpuZ180, z) == z80_len[i]);
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ80, z) == 2);
		}
	}
	...
	SUBCASE("")
	{
		...
		{
			uint8 z[4] = {0xcb, uint8(i), 0, 0};
			CHECK(opcode_length(CpuZ180, z) == 2);
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ80, z) == 4);
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ180, z) == 4); // sll as for z80 too
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ80, z) == z80_ix_len[i]);
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ180, z) == z80_ix_len[i]);
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ80, z) == z80_ed_len[i]);
		}
	}
	...
	SUBCASE("")
	{
		for (uint i = 0; i < 256; i++)
		{
			...
			CHECK(opcode_length(CpuZ180, z) == z180_ed_len[i]);
		}
	}

	SUBCASE("")
	{
		static const Byte ld_a_xix[] = {0xdd, LD_A_xHL, 0};
		CHECK(opcode_length(CpuZ180, ld_a_xix) == 3);
		static const Byte ld_xix_a[] = {0xdd, LD_xHL_A, 0};
		CHECK(opcode_length(CpuZ180, ld_xix_a) == 3);
		static const Byte ld_b_xix[] = {0xdd, LD_B_xHL, 0};
		CHECK(opcode_length(CpuZ180, ld_b_xix) == 3);
		static const Byte ld_xix_b[] = {0xdd, LD_xHL_B, 0};
		CHECK(opcode_length(CpuZ180, ld_xix_b) == 3);
		static const Byte ld_c_xix[] = {0xdd, LD_C_xHL, 0};
		CHECK(opcode_length(CpuZ180, ld_c_xix) == 3);
		static const Byte ld_xix_c[] = {0xdd, LD_xHL_C, 0};
		CHECK(opcode_length(CpuZ180, ld_xix_c) == 3);
		static const Byte ld_ix_halt[] = {0xdd, HALT, 0};
		CHECK(opcode_length(CpuZ180, ld_ix_halt) == 2);
		static const Byte or_xix[] = {0xdd, OR_xHL, 0};
		CHECK(opcode_length(CpuZ180, or_xix) == 3);
		static const Byte xor_xix[] = {0xdd, XOR_xHL, 0};
		CHECK(opcode_length(CpuZ180, xor_xix) == 3);
		static const Byte or_xh[] = {0xdd, OR_H, 0};
		CHECK(opcode_length(CpuZ80, or_xh) == 2);
		static const Byte xor_xl[] = {0xdd, XOR_L, 0};
		CHECK(opcode_length(CpuZ80, xor_xl) == 2);
	}
}
