...
#define wmh (wm >> 8)  // access high byte of wm
	...
	case HALT:
		pc--;
		if (halt) LOOP; // 4 T  ((executes NOPs))
		halt = yes;
		Z80_INFO_HALT;
		LOOP;
	...
	ld_nn:
		GET_N(rzl);
		GET_N(rzh);
		LOOP; // timing: pc:4,pc+1:3,pc+2:3
	...
	jr:
		...
		SKIP_5X1CC(pc);
		pc += (int8)c;
		LOOP; // timing: pc:4, pc+1:3,5*1
	case JR_Z:
		if (rf & Z_FLAG) goto jr; // timing: pc:4, pc+1:3,[5*1]
	njr:
		SKIP_N();
		LOOP;
	case JR_C:
	...
	case LD_HL_xNN:
		...
		PEEK(RL, w);
		PEEK(RH, w + 1);
		LOOP; // timing: pc:4, pc+1:3, pc+2:3, nn:3, nn+1:3
	...
	pop_rr:
		POP(rzl);
		POP(rzh); // timing: pc:4, sp:3, sp+1:3
		goto pop_af;
	case POP_AF:
		POP(rf);
		POP(ra); // timing: pc:4, sp:3, sp+1:3
	pop_af:
		Z80_INFO_POP;
		LOOP;
	...
	call:
		GET_NN(w);
		SKIP_1CC(pc - 1); // pc:4, pc+1:3, pc+2:3,1, sp-1:3, sp-2:3
	rst:
		PUSH(pc >> 8);
		PUSH(pc);
		pc = w;
	...
	case DEC_xHL:
		...
		SKIP_1CC(w);
		M_DEC(c);
		POKE_AND_LOOP(w, c); // pc:4, hl:3,1, hl:3
	case INC_xHL:
		w = HL;
		PEEK(c, w);
		SKIP_1CC(w);
	...
	case LD_xNN_HL:
		GET_NN(w);
		POKE(w, RL);
		POKE_AND_LOOP(w + 1, RH); // pc:4, pc+1:3, pc+2:3, nn:3, nn+1:3
	...
	case EX_HL_xSP:
		w = HL; // pc:4, sp:3, sp+1:3,1, sp+1:3, sp:3,2x1
		PEEK(RL, SP);
		PEEK(RH, SP + 1);
		SKIP_1CC(SP + 1); //						(kio tested 2005-01-15)
		POKE(SP + 1, wh);
		POKE(SP, wl);
		SKIP_2X1CC(SP);
		Z80_INFO_EX_HL_xSP;
	...
	XY: // 4 T
		...
		case PUSH_HL:
			...
			PUSH(rzh);
			PUSH(rzl);
			LOOP; // 4+ pc:5, sp-1:3, sp-2:3
		case POP_HL:
			POP(rzl);
			POP(rzh);
			LOOP; // 4+ pc:4, sp:3, sp+1:3
		...
		case LD_xNN_HL:
			GET_NN(w);
			POKE(w, rzl);
			POKE_AND_LOOP(w + 1, rzh); // 4+ pc:4, pc+1:3, pc+2:3, nn:3, nn+1:3
		case LD_HL_xNN:
			GET_NN(w);
			PEEK(rzl, w);
			PEEK(rzh, w + 1);
			LOOP; // 4+ pc:4, pc+1:3, pc+2:3, nn:3, nn+1:3
		...
		case EX_HL_xSP: // pc:4, pc+1:4, sp:3, sp+1:3,1, sp+1:3, sp:3,2x1
			...
			POKE(SP + 1, w >> 8);
			POKE(SP, w);
			SKIP_2X1CC(SP); // kio tested 2005-01-15
			Z80_INFO_EX_HL_xSP;
			LOOP;
			...
			case LD_xHL_N:
				GET_N(dis);
				GET_N(c);
				SKIP_2X1CC(pc - 1); // timing:	pc:4, pc+1:4, pc+2:3, pc+3:3,2x1, ix+n:3 (korr kio 2005-01-15)
				POKE_AND_LOOP(rz + dis, c);
			case DEC_xHL:
				GET_N(dis);
				SKIP_5X1CC(pc - 1);
				w = rz + dis; // timing:	pc:4, pc+1:4, pc+2:3, pc+2:1x5, ix+n:3,1, ix+n:3
				PEEK(c, w);
				SKIP_1CC(w);
				M_DEC(c);
				POKE_AND_LOOP(w, c);
			...
			case ADD_xHL:
				...
				PEEK(c, w);
				M_ADD(c);
				LOOP; // pc:4, pc+1:4, pc+2:3, pc+2:1x5, ix+n:3
			case SUB_xHL:
				GET_N(dis);
				SKIP_5X1CC(pc - 1);
				w = rz + dis;
	...
	case PFX_ED: // 4 T: Timing berücksichtigt im ED-Dispatcher
		...
		case LD_BC_xNN:
			...
			PEEK(RC, w);
			PEEK(RB, w + 1);
			LOOP; // timing: pc:4, pc+1:4, pc+2:3, pc+3:3, nn:3, nn+1:3
		case LD_DE_xNN:
			GET_NN(w);
			PEEK(RE, w);
			PEEK(RD, w + 1);
	...
	*/
		case RETI:
			Z80_INFO_RETI; // timing: pc:4, pc+1:4, sp:3, sp+1:3
		...
		case RETN:
			...
			case RRD:
				w = HL;
				PEEK(o, w); // timing: pc:4, pc+1:4, hl:3,4*1, hl:3
				c  = (o >> 4) + (ra << 4);
				ra = (ra & 0xF0) + (o & 0x0F);
				goto rld;
		...
		ldir:
			PEEK(c, HL); // timing:  pc:4, pc+1:4, hl:3, de:3,2x1 [de:5x1] (kio tested 2005-01-15)
			POKE(DE, c);
		...
		ldi:
			PEEK(c, HL);
			HL += w; // timing:  pc:4, pc+1:4, hl:3, de:3,2x1  (kio tested 2005-01-15)
			POKE(DE, c);
		...
		cpir:
			PEEK(c, w);
			c = ra - c; // timing: pc:4, pc+1:4, hl:3,5*1,[5*1]  (kio verified 2005-01-10)
			SKIP_5X1CC(w);
			BC -= 1;
		...
		cpi:
			PEEK(c, w);
			c = ra - c; // timing: pc:4, pc+1:4, hl:3,5*1
			SKIP_5X1CC(w);
			BC -= 1;
			rf = (rf & C_FLAG) + (c & S_FLAG) + (c ? 0 : Z_FLAG) + N_FLAG + (BC ? P_FLAG : 0) +
				 ((ra ^ (ra - c) ^ c) & H_FLAG);
		...
		inir:
			INCR_CC(1); // timing: pc:4, pc+1:5, IO, hl:3,[5*1]
			INPUT(BC, c);
			POKE(w, c);
			if (--RB)
			{
		...
		ini:
			INCR_CC(1); // timing: pc:4, pc+1:5, IO, hl:3
			INPUT(BC, c);
		...
		otir:
			INCR_CC(1); // timing:  pc:4, pc+1:5, hl:3, IO, [hl:5*1]
			PEEK(c, w);
		...
		outi:
			INCR_CC(1); // timing: pc:4, pc+1:5, hl:3, IO
			PEEK(c, w);
			--RB;		   // kio 2005-11-18: OUTI: decr B before putting it on the bus
			OUTPUT(BC, c); // [post on css by Alvin Albrecht]
			rf = N_FLAG + (RB ? 0 : Z_FLAG);
